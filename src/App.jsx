...
// ★ 簡易ログイン用パスコード（必要に応じて変更してください）
const SYSTEM_PASSCODE = '1234'; 

export default function App() {
  // 認証ステート（ローカルストレージに保持してリロード時もログイン状態を維持）
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    if (inputPasscode === SYSTEM_PASSCODE) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-sm bg-slate-800 rounded-3xl p-6 shadow-2xl border border-slate-700 space-y-6">
          <div className="text-center space-y-2">
            ...
            <h1 className="text-xl font-black text-white">ホテル消耗品 在庫Navi</h1>
            <p className="text-xs text-slate-400">スタッフ専用パスコードを入力してください</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-base text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition tracking-widest text-center"
                  autoFocus
                />
              </div>
              {loginError && (
                <p className="text-rose-400 text-xs font-bold text-center mt-2 animate-bounce">
                  パスコードが違います
                </p>
              )}
            </div>
            ...
            <button
              type="submit"
              className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white font-bold rounded-xl transition shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-2 text-sm"
            >
        ...
        <div className="max-w-md mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className="p-1.5 bg-indigo-800/80 hover:bg-rose-600 text-indigo-200 hover:text-white rounded-xl border border-indigo-500/40 transition"
              title="ログアウト"
            >
